...
The stats were then taken to compute how many fixation events were eliminated or combined during initial cleaning by Data Viewer.
...
1. Lexical predictability values were then referenced from the Provo Corpus and inserted as a variable.
1. The variable `windowcondition` was changed from a numeric to a factor.
1. `Text_ID` was changed from a string to a factor.
1. `IA_ID` was changed from a numeric to a factor.
1. The names of the levels for `windowcondition` were changed such that 0 became "No Preview" and 1 became "Preview" for ease of comparison and graphing later.
...
sumStats2 <- COMBINED %>%
  ...
  gather(Statistic, Value, "First Fixation Duration":"Dwell Time") %>%
  spread(windowcondition, Value)

# need to add single fixation duration, total time, gaze duration.
...
I constructed some visualizations of the effect of window condition and text predictability on first fixation duration, dwell time, and location of first fixation. The most pronounced effects are for the first fixation, with little apparent effect for location.
...
### Model for First Fixation Position
```{r}
