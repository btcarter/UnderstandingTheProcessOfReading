...
####  Changes from V1
1. The number of characters in each word was entered as a separate variable (`Word_Length`).
...
Word length (Word_Length) has been added to test for an interaction.
...
`log(IA_DWELL_TIME) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * Word_Length + (1 + windowcondition|RECORDING_SESSION_LABEL) + (1|WORD_ID)`
...
# )
  ...
  # third model
mod3 <- lmer(
  log(IA_DWELL_TIME) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * Word_Length +
    (1 + windowcondition|RECORDING_SESSION_LABEL) +
    (1|WORD_ID),
  data = COMBINED %>% filter(IA_DWELL_TIME > 0)
)
