...
FIXS <- read_xls(file.path(DIR,"FixList.xls")) # list of fixations
```
...
TEST=$(tail -2 cleaning-UPoR1.txt | head -1 | sed -E 's/.*(\d*)/\1/g')
TEST=$(tail -2 cleaning-UPoR1.txt | head -1 | sed -E 's/\D+(\d+)/\1/g')
```
