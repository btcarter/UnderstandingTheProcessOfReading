...
####  Changes from V1
1. The number of characters in each word was entered as a separate variable (`N_CHAR`).
...
# word length
COMBINED$N_CHAR <- nchar(as.character(COMBINED$IA_LABEL))

# remove words that were not skipped the first time through.
COMBINED <- COMBINED[COMBINED$IA_SKIP != 1,]
...
Word length (N_CHAR) has been added to test for an interaction.
...
`log(IA_FIRST_FIXATION_DURATION) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR + (1 + windowcondition|RECORDING_SESSION_LABEL) + (1|WORD_ID)`
...
mod1 <- lmer(
  log(IA_FIRST_FIXATION_DURATION) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR +
...
`IA_FIRST_FIXATION_DURATION) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR + (1|RECORDING_SESSION_LABEL) + (1|WORD_ID)`
...
`IA_FIRST_RUN_DWELL_TIME ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR + (1 + windowcondition|RECORDING_SESSION_LABEL) + (1|WORD_ID)`
...
# gaze duration 4
mod5 <- lmer(
   IA_FIRST_RUN_DWELL_TIME ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR +
     (1|RECORDING_SESSION_LABEL) +
     (1|WORD_ID),
   data = COMBINED %>% filter(IA_DWELL_TIME > 0)
 )
...
`log(IA_DWELL_TIME) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR + (1 + windowcondition|RECORDING_SESSION_LABEL) + (1|WORD_ID)`
...
# )
  ...
  # third model
mod3 <- lmer(
  log(IA_DWELL_TIME) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) * N_CHAR +
    (1 + windowcondition|RECORDING_SESSION_LABEL) +
    (1|WORD_ID),
  data = COMBINED %>% filter(IA_DWELL_TIME > 0)
)
