...
```{r dataViewer_stats, echo=TRUE, error=FALSE, message=FALSE}
MERGED <- 738
DELETED <- 9985
...
```{r preprocessing, error=FALSE, message=FALSE, echo=TRUE}
# read in the data
DIR <- file.path("~","Box","LukeLab","UnderstandingTheProcessOfReading","data") # study directory
...
```{r summary_stats, error=FALSE, message=FALSE, echo=TRUE}
...
```{r trends, error=FALSE, message=FALSE, echo=TRUE}
...
`log(IA_FIRST_FIXATION_DURATION) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) + Word_Length + (1 + windowcondition|RECORDING_SESSION_LABEL) + (1|WORD_ID)`

```{r fixationDuration_1, error=FALSE, message=FALSE, echo=TRUE}
...
mod1 <- lmer(
  log(IA_FIRST_FIXATION_DURATION) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) + Word_Length +
...
`IA_FIRST_FIXATION_DURATION) ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) + Word_Length + (1|RECORDING_SESSION_LABEL) + (1|WORD_ID)`

```{r fixationDuration_2, error=FALSE, message=FALSE, echo=TRUE}
...
`IA_FIRST_RUN_DWELL_TIME ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) + Word_Length + (1 + windowcondition|RECORDING_SESSION_LABEL) + (1|WORD_ID)`

```{r gazeDuration, error=FALSE, message=FALSE, echo=TRUE}
...
# gaze duration 4
mod5 <- lmer(
   IA_FIRST_RUN_DWELL_TIME ~ windowcondition * scale(log(mean_OrthoMatchModel), scale = FALSE) + Word_Length +
     (1|RECORDING_SESSION_LABEL) +
     (1|WORD_ID),
   data = COMBINED %>% filter(IA_DWELL_TIME > 0)
 )
...
```{r dwellTime, error=FALSE, message=FALSE, echo=TRUE}
